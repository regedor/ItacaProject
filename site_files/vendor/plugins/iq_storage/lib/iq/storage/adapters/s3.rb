...
module IQ::Storage::Adapters # :nodoc:
  class S3Config
...
#      AWS::S3::S3Object.store(storage_path(model_instance), model_instance.temp_data, self.config.bucket_name,
...
#      if File.file?(fs_adapter.full_filename(model_instance))
#        return fs_adapter.fetch(model_instance)
#      else
...
#    end
#
#    # the path to the asset file within your apps public directory
#    def path(model_instance, version=nil)
#      File.join("http://s3.amazonaws.com/", self.config.bucket_name, model_instance.storage_path)
#    end
#
#    def file_data(model_instance)
#      AWS::S3::S3Object.value(storage_path(model_instance),self.config.bucket_name)
#    end
#
#    def remove_from_remote_storage(model_instance)
#      AWS::S3::S3Object.erase(storage_path(model_instance,model_instance.version_name), self.config.bucket_name)
#    end
  end
end